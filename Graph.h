...
struct Rectangle_BS : Shape {

	Rectangle_BS(Point xy, int ww, int hh) :w{ ww }, h{ hh }
	{
		if (h<=0 || w<=0) error("Bad rectangle: non-positive side");
		add(xy);
	}
	Rectangle_BS(Point x, Point y) :w{ y.x - x.x }, h{ y.y - x.y }
	{
...
struct Polygon_BS : Closed_polyline {	// closed sequence of non-intersecting lines
...
private:
	string lab;	// label
	Font fnt{ fl_font() };
	int fnt_sz{ (14<fl_size()) ? fl_size() : 14 };	// at least 14 point??????????????я исправил
...
struct Ellipse_BS : Shape {
	Ellipse_BS(Point p, int ww, int hh)	// center, min, and max distance from center
	:w{ ww }, h{ hh } {
...
// struct Arc : Ellipse_BS {
	// using Ellipse_BS::Ellipse_BS;
	// void draw_lines() const;
// };
